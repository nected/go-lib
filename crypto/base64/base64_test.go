...
func TestB64EncodeURL(t *testing.T) {
	tests := []struct {
		input    []byte
		expected string
	}{
	...
	for _, test := range tests {
		result := B64EncodeURL(test.input)
		if result != test.expected {
			t.Errorf("B64EncodeURL(%q) = %q; want %q", test.input, result, test.expected)
		}
	}
}
func TestB64DecodeURL(t *testing.T) {
	tests := []struct {
		input    string
		expected string
		hasError bool
	...
	for _, test := range tests {
		result, err := B64DecodeURL(test.input)
		if (err != nil) != test.hasError {
			t.Errorf("B64DecodeURL(%q) error = %v; want error = %v", test.input, err != nil, test.hasError)
		}
		if result != test.expected {
			t.Errorf("B64DecodeURL(%q) = %q; want %q", test.input, result, test.expected)
		}
	}
}
